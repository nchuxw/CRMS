				...
				<div class="row">
					<div class="col-sm-9">
						<form name="frm" class="form-inline"
							action="RoomadminManageServlet" method="post">
							<table>
								<tr>
									<td>
										<div class="form-group">
											<label class="form-label">工号：</label><input
												class="form-control" type="text" name="userNo"
												v-model="currentData.loanRate" maxlength="8"
												onkeyup="this.value=this.value.replace(/[^0-9.]/g,'')"
												value='<c:out value="${userNo}"></c:out>'>
										</div>
									</td>
									<td>
										<div class="form-group">
											<label class="form-label">姓名：</label><input
					...
					<div class="btn-group col-sm-3">
						<button type="button" class="btn btn-info dropdown-toggle"
							...
							onclick='window.location="roomadmin_add.jsp"'
							aria-haspopup="true" aria-expanded="false">添加</button>
					</div>
				</div>

				<div class="table-responsive">
					<form onSubmit="checkForm()" action="RoomadminsDeleteServlet">
						<p>&nbsp;</p>
						<input type="submit" class="btn btn-primary" value="批量删除" />
						<table class="table table-striped">
							<thead>
								<tr>
									<th><input type="checkbox" onclick="cheAll()" id="che"
										name="chk_uid"> <label class="form-label">全选</label></th>
									<th>工号</th>
									<th>姓名</th>
									<th>用户类型</th>
									<th>所属班级</th>
							...
							<tbody>
								<c:forEach items="${list }" var="user">
									<tr>
										<td><input type="checkbox" onclick="" name="ck"
										...
										<td><a class="btn btn-primary"
											href='RoomadminEditServlet?id=${user.userId}'>修改</a> <a
											class="btn btn-danger"
											href='RoomadminDeleteServlet?id=${user.userId}'>删除</a></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
